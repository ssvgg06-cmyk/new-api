...
const OtherSetting = () => {
  ...
  const checkUpdate = async () => {
    try {
      ...
      const res = await fetch(
        'https://api.github.com/repos/ssvgg06-cmyk/new-api/releases/latest',
        {
          headers: {
            Accept: 'application/json',
            'Content-Type': 'application/json',
            // Adding User-Agent which is often required by GitHub API
            'User-Agent': 'tinimo-update-checker',
          },
        },
  ...
  const openGitHubRelease = () => {
    window.open(
      `https://github.com/ssvgg06-cmyk/new-api/releases/tag/${updateData.tag_name}`,
      '_blank',
    );
  };
